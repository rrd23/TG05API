...
async def start(message: Message):
    await message.answer("Привет! Я бот, который поможет тебе найти котика по породе.")
...
async def send_cat_info(message: Message):
    ...
    if breed_info:
        cat_image_url = get_cat_image_by_breed(breed_info['id'])  # Убрали await
        await message.answer_photo(cat_image_url)
        info = f"Порода: {breed_info['name']}\n"
        info += f"Описание: {breed_info['description']}\n"
        info += f"Продолжительность жизни: {breed_info['life_span']}\n"
